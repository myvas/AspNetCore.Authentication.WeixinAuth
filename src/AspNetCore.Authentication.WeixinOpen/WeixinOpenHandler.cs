...
using AspNetCore.Authentication.WeixinOpen.Extensions;
...
{
    internal class WeixinOpenHandler<TOptions> : RemoteAuthenticationHandler<TOptions>
        where TOptions : WeixinOpenOptions, new()
    {
        ...
        /// </summary>
        protected new WeixinOpenEvents<TOptions> Events
        {
            get => (WeixinOpenEvents<TOptions>)base.Events;
        ...
        protected override Task<object> CreateEventsAsync() => Task.FromResult<object>(new WeixinOpenEvents<TOptions>());

        public WeixinOpenHandler(IOptionsMonitor<TOptions> options, ILoggerFactory logger, UrlEncoder encoder, ISystemClock clock)
        ...
        protected override async Task HandleChallengeAsync(AuthenticationProperties properties)
        {
            ...
            var redirectContext = new WeixinOpenRedirectToAuthorizationContext<TOptions>(
        ...
        /// <returns></returns>
        protected virtual async Task<WeixinOpenTokenResponse> CustomExchangeCodeAsync(string code, string redirectUri)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                return WeixinOpenTokenResponse.Failed(new Exception(error));
            }
            var payload = JObject.Parse(await response.Content.ReadAsStringAsync());
            var result = WeixinOpenTokenResponse.Success(payload);
            //错误时微信会返回错误JSON数据包，示例如下: { "errcode":40029,"errmsg":"invalid code"}
            if (string.IsNullOrWhiteSpace(result.AccessToken))
            {
                int errorCode = WeixinOpenHandlerHelper.GetErrorCode(payload);
                var errorMessage = WeixinOpenHandlerHelper.GetErrorMessage(payload);
                return WeixinOpenTokenResponse.Failed(new Exception($"The remote server returned an error while exchanging the code. {errorCode} {errorMessage}"));
            }
            return result;
        }
        ...
        protected virtual async Task<AuthenticationTicket> CustomCreateTicketAsync(
            ClaimsIdentity identity, AuthenticationProperties properties, WeixinOpenTokenResponse tokens)
        {
            if (identity == null)
            {
            ...
            identity.AddOptionalClaim(WeixinOpenClaimTypes.OpenId, openId, Options.ClaimsIssuer);
            // scope
            identity.AddOptionalClaim(WeixinOpenClaimTypes.Scope, scope, Options.ClaimsIssuer);

            if (SplitScope(scope).Contains(WeixinOpenScopes.snsapi_login))
            {
            ...
            //TODO: generic type
            var context = new WeixinOpenCreatingTicketContext<WeixinOpenOptions>(Context, Scheme, Options, Backchannel, ticket, tokens);
        ...
        private async Task<ClaimsIdentity> GetUserInformationAsync(string accessToken, string openId, ClaimsIdentity identity)
        {
            ...
            else
            {
                //提取userinfo
                // std:Name
                var nickname = WeixinOpenHandlerHelper.GetNickName(payload);
                identity.AddOptionalClaim(ClaimTypes.Name, nickname, this.Options.ClaimsIssuer);
                identity.AddOptionalClaim(WeixinOpenClaimTypes.NickName, nickname, Options.ClaimsIssuer);

                var sex = WeixinOpenHandlerHelper.GetGender(payload);
                identity.AddOptionalClaim(WeixinOpenClaimTypes.Gender, sex, Options.ClaimsIssuer);

                var province = WeixinOpenHandlerHelper.GetProvince(payload);
                identity.AddOptionalClaim(WeixinOpenClaimTypes.Province, province, Options.ClaimsIssuer);

                var city = WeixinOpenHandlerHelper.GetCity(payload);
                identity.AddOptionalClaim(WeixinOpenClaimTypes.City, city, Options.ClaimsIssuer);

                var country = WeixinOpenHandlerHelper.GetCountry(payload);
                identity.AddOptionalClaim(WeixinOpenClaimTypes.Country, country, Options.ClaimsIssuer);

                var headImageUrl = WeixinOpenHandlerHelper.GetHeadImageUrl(payload);
                identity.AddOptionalClaim(WeixinOpenClaimTypes.HeadImageUrl, headImageUrl, Options.ClaimsIssuer);

                var privileges = WeixinOpenHandlerHelper.GetPrivileges(payload);
                foreach (string privilege in privileges)
                {
                    identity.AddOptionalClaim(WeixinOpenClaimTypes.Privilege, privilege, Options.ClaimsIssuer);
                }

                var unionId = WeixinOpenHandlerHelper.GetUnionId(payload);
                identity.AddOptionalClaim(WeixinOpenClaimTypes.UnionId, unionId, Options.ClaimsIssuer);
            }
